...
exports.getCombinedInventoryReport = async (req, res) => {
  try {
    ...
    if (!user_id || !business_id) {
      return res.status(400).json({ message: "user_id and business_id are required" });
    }
    ...
    const query = `
      ...
      LEFT JOIN invoice_items ii ON ii.item_id = it.id
      LEFT JOIN invoices i ON i.id = ii.invoice_id
      WHERE it.user_id = ? AND it.business_id = ?
      GROUP BY it.id;
    `;

    const [rows] = await db.query(query, [user_id, business_id]);
    ...
    rows.forEach((item, index) => {
      const sn = index + 1;
      const hsn = '25010101'; // You can replace this with dynamic HSN if available
